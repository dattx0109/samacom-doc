(function ($) {
    ...
    let Job = (function ($, window, document) {
        ...
        dataRequest.error_job_type = $('div.error_job_type');
        dataRequest.error_field_work_sale = $('div.error_field_work_sale');
        dataRequest.error_workplace_full_text = $('div.error_workplace_full_text');
        dataRequest.error_level_id = $('div.error_level_id');
    ...
    Job.btnStore.on('click', function () {
        ...
        jobInsert.salary_base_type = Job.salary_base_type.is(":checked")?1:2;
        jobInsert.income_type = Job.income_type.is(":checked")?1:2;
        let addJob = $.post('/job/store', jobInsert);
        addJob.done(function (data) {
            setTimeout(function () {
                toastr.options = {
                    closeButton: true,
                    progressBar: true,
                ...
                toastr.success('', 'Tạo Job thành công');

            }, 500);
            setTimeout(function () {
                 window.location.href = "/danh-sach-cong-viec";

            }, 2000);

        });
        addJob.fail(function (data) {
            ...
            if (typeof errors.job_type != 'undefined') {
                 Job.error_job_type.append('<span class="help-block m-b-none alert alert-danger">'+errors.job_type[0]+'</span>');
            }
            ...
            if (typeof errors.level_id != 'undefined') {
                 Job.error_level_id.append('<span class="help-block m-b-none alert alert-danger">'+errors.level_id[0]+'</span>');
            }
            ...
            if (typeof errors.type != 'undefined') {
                Job.error_type.append('<span class="help-block m-b-none alert alert-danger">'+errors.type[0]+'</span>');
            }
            if (typeof errors.tag != 'undefined') {
                Job.error_tag.append('<span class="help-block m-b-none alert alert-danger">'+errors.tag[0]+'</span>');
            }
            if (typeof errors.workplace_full_text != 'undefined') {
                Job.error_workplace_full_text.append('<span class="help-block m-b-none alert alert-danger">'+errors.workplace_full_text[0]+'</span>');
            }
        });
    });
    ...
    Job.salary_base_type.on('change', function () {
        if ($(this).is(":checked")) {
            Job.base_salary.hide();
        }
        if (!$(this).is(":checked")) {
            Job.base_salary.show();
        }
    });
